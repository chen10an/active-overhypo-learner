...
object Conditioner {
  ...
  def getPhaseResults(learner: PhaseLearner, events: Vector[Event]): (Array[(Array[Double], Array[Double], Double, Double, Double, Map[Fform, Double], Map[Set[Block], Double], Map[Hyp, Double])], Array[Set[Block]], Dist[Hyp]) = {
    // returns:
    ...
    var results = Array.ofDim[(Array[Double], Array[Double], Double, Double, Double, Map[Fform, Double], Map[Set[Block], Double], Map[Hyp, Double])](events.length)
